...
*   `indicator`（指标选择）：UI 上呈现为下拉选择器，选取预设的监测指标名称。
*   `time_range`（时间跨度）：UI 上呈现为日期或相对时间距离。
*   `scope`（空间范围）：UI 上呈现为设备或子系统组件树。
*   `threshold`（设定阈值）：用于填写警报线或评判标准，如 `80%`，`0.5mm`。
*   `operator`（比较操作符）：如 `>=`、`<`、`包含`、`等于`，用于与阈值搭配形成判断逻辑。
*   `enum_select`（通用枚举选择）：用于风险等级选择（高/中/低）、报警级别等固定枚举集合。
*   `number`（纯数值）：单纯的数值输入。
*   `boolean`（开关量）：如是否包含某个特定类型的特征。
